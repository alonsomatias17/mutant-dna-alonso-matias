...
public class MutantController {
    ...
    public ResponseEntity<String> mutantIdentifier(@RequestBody RequestObject requestObject) {
        ...
        if (mutantIdentificationService.isMutant( requestObject.getDna())) {
            mutantStatService.update(DnaSequence.DNA_TYPE_MUTANT, Arrays.toString(requestObject.getDna()) );
            return ResponseEntity.status(HttpStatus.OK).build();

        }
        mutantStatService.update(DnaSequence.DNA_TYPE_HUMAN, Arrays.toString(requestObject.getDna()) );
